...
model = Sequential()
model.add(Bidirectional(LSTM(units=64,  return_sequences=True), input_shape=in_dim))
model.add(Bidirectional(LSTM(units=32,  return_sequences=True), input_shape=in_dim))
model.add(Bidirectional(LSTM(units=16), input_shape=in_dim))
